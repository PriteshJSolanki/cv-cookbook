...
# # Visualize an image
# # visualize_img(X_train[3], y_train[3][0])
...
# # CONV/POOL_1
# model.add(Conv2D(filters=filters, kernel_size=kernel_size, padding=padding, activation='relu',
#                  input_shape=(32,32,3)))
# model.add(MaxPooling2D(pool_size))

# # CONV/POOL_2
# model.add(Conv2D(filters=filters*2, kernel_size=kernel_size, padding=padding, activation='relu',
#                  input_shape=(32,32,3)))
# model.add(MaxPooling2D(pool_size))

# # CONV/POOL_3
# model.add(Conv2D(filters=filters*4, kernel_size=kernel_size, padding=padding, activation='relu',
#                  input_shape=(32,32,3)))
# model.add(MaxPooling2D(pool_size))
...
# # Train
# checkpoint = ModelCheckpoint(filepath='./models/cnn_cifar10.weights.best.hdf5', verbose=1, save_best_only=True)
# early_stop = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=25)  
...
# #  Eval
# #################################
# model.load_weights('./models/cnn_cifar10.weights.best.hdf5')

# score = model.evaluate(X_test, y_test, verbose=0)
# acc = round(100 * score[1],2)
# print(f'Model Accuracy: {acc}')

# # Visualize the some of the data
# predictions = model.predict(X_test)
# fig = plt.figure(figsize=(20,8))
# samples = np.random.choice(X_test.shape[0], size=32, replace=False)
# for i, sample_num in enumerate(samples):
#     ax = fig.add_subplot(4,8, i+1, xticks=[], yticks=[])
#     ax.imshow(np.squeeze(X_test[sample_num]))
#     pred = np.argmax(predictions[sample_num])
#     actual = np.argmax(y_test[sample_num])
#     ax.set_title("{} ({})".format(cifar10_labels[pred], cifar10_labels[actual]),
#                  color=("green" if pred == actual else "red"))